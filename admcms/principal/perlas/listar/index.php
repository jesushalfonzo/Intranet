...
<? 
	   $link=Conectarse();   
	   $tirasql="select * from perlas order by id_perlas desc";
       $resultado=mysql_query($tirasql,$link); 
  while ($row = mysql_fetch_array($resultado)){ 
     $idperlas=$row['id_perlas'];
     $tituloperlas=$row['titulo_perlas'];
	 $archivoperlas=$row['archivo_perlas'];
	 
?>
<table width="515" border="0" cellpadding="3" cellspacing="0" class="texto">
    <tr>
    <td bgcolor="#FFFFFF" class="etiquetas"><? echo "$tituloperlas"; ?>  </td>
      <td align="right" bgcolor="#FFFFFF" class="etiquetas"><table border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td> 
<input name="editar" type="submit" class="pequeno" id="editar" value="Editar"  onClick="TomarValor('<? echo "$idperlas";?>','Editar')" />
         </td>
          <td>
<input name="borrar" type="submit" class="pequeno" id="borrar" value="Borrar"  onClick="TomarValor('<? echo "$idperlas";?>','Eliminar')" />
</td>
        </tr>
      </table>
</td>
