...
<title>Listar Lecturas Recomendadas</title>
...
<? 
	   $link=Conectarse();   
	   $tirasql="select * from lecturas order by id_lectura DESC";
       $resultado=mysql_query($tirasql,$link); 
  while ($row = mysql_fetch_array($resultado)){ 
     $id_lectura=$row['id_lectura'];
     $titulo=$row['titulo_lectura'];
...
<table width="515" border="0" cellpadding="3" cellspacing="0" class="texto">
    <tr>
    <td bgcolor="#FFFFFF" class="etiquetas"><? echo "$titulo"; ?>  </td>
      <td align="right" bgcolor="#FFFFFF" class="etiquetas"><table border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td> 
<input name="editar" type="submit" class="pequeno" id="editar" value="Editar"  onClick="TomarValor('<? echo "$id_lectura";?>','Editar')" />
         </td>
          <td>
<input name="borrar" type="submit" class="pequeno" id="borrar" value="Borrar"  onClick="TomarValor('<? echo "$id_lectura";?>','Eliminar')" />
</td>
        </tr>
      </table>
</td>
