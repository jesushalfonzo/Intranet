...
<table width="788" border="1" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; color:#FFFFFF" align="center">
  ...
  <tr bgcolor="#000000">
    ...
    <td width="356" align="center"><span class="letrasblancas">Mensaje</span></td>
    <td width="107" ><div align="center" class="letrasblancas">Fecha de envio </div></td>
   
  
    <td width="244" ><div align="center" class="letrasblancas">Usuario</div></td>
  ...
  $link=Conectarse();
    ...
    $tirasql="SELECT * FROM m_bitacora_sms, m_usuarios WHERE m_bitacora_sms_usu = m_usuarios_id ORDER BY m_bitacora_sms_fecha DESC LIMIT 0,15";
...
<?php
    ...
    while ($row = mysql_fetch_array($result)){ 
     $m_bitacora_sms_id=$row['m_bitacora_sms_id'];
	 $m_bitacora_sms_fecha=strtotime($row['m_bitacora_sms_fecha']);
	 $fecha = date("d-m-Y H:i", $m_bitacora_sms_fecha);
	 $m_bitacora_sms_texto=$row['m_bitacora_sms_texto'];
	 $m_bitacora_sms_usu=$row['m_bitacora_sms_usu'];
